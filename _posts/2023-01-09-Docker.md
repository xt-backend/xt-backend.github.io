...
* docker image tag MyImageId:MyImageTag NewImageId:NewImageTag
  
  * 특정 Image의 ImageId와Tag를 변경
...
* docker run [options] [ImageId | ImageName]
  ...
  * -d :  컨테이너 백그라운드에서 실행
  ...
  ```docker
  docker run -d MyImageId|MyImageName
  ```
  
  * -p : 호스트와 컨테이너 간의 포트 바인딩
  
  ```docker
  docker run -p HostPcPort : ContainerPort MyImageId|MyImageName 
  ```
  
  * --name : 만들어질 Container의 name 설정
  
  ```docker
  docker run --name MyContainerName MyImageId|MyImageName
  ```
  
  * -volume | -v : HOST PC의 파일과 Container 내부의 파일 연동
  
  ```docker
  docker run -v HostFileDirectory:ContainerFileDirectory MyImageId|MyImageName
  ```
  
  * -e : 컨테이너의 환경변수 설정
  
  ```docker
  docker run -e 환경변수 Key값=환경변수값 MyImageId|MyImageName
  ```
...
* Docker logs ContainerId [options]
  ...
  * 관련 options
    ...
    * --tail [출력할 Line 수] : 마지막 로그부터 사용자가 지정한 Line수까지 출력
    ...
    ```docker
    docker logs --tail 10 MyContainerId|MyContainerName
    ```
...
* Docker exec ContainerId [options]
...
* docker ps vs docker container ls
  ...
  * 차이점
    ...
    * 두 가지 모두 현재 사용 가능하지만 Docker 공식자료에서는 명령어의 의미를 좀더 명확히 하기위해 container ls, image ls등의 명령어를 새로 만들었다고 말함
